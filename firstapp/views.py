...
from .models import Author_db, Exhibition_db, Card_db, Organization, User, Control, Directors, Fund_manager, Users
from .forms import AddData, AddExh, AddCard, AddOrg, AddUser, AddControl, AddDirector, AddManager
...
def index_user(request):
    form_ca = AddUser()
    data = Users.objects.all()
    return render(request, "firstapp/Users/Template_Users.html", {"form":form_ca, "data_show":data})

def index_director(request):
    form_ca = AddDirector()
    data = Directors.objects.all()
    return render(request, "firstapp/Users/Template_Director.html", {"form":form_ca, "data_show":data})

def index_manager(request):
    form_ca = AddManager()
    data = Fund_manager.objects.all()
    return render(request, "firstapp/Users/Template_Manager.html", {"form":form_ca, "data_show":data})
...
class view_move(View):
    def add_move(request):
        if request.method == "POST":
            ...
            new_author.organization_id = Organization.objects.get(id=request.POST.get("organization_id"))
...
class view_user(View):
    ...
    def add_director(request):
        if request.method == "POST":
            user = Directors()
            user.users_id = Users.objects.get(id=request.POST.get("users_id"))
            user.phone = request.POST.get("phone")
            user.position = request.POST.get("position")
            user.save()
            return HttpResponseRedirect("/user/director")
    ...
    def del_director(request):
        if request.method == "POST":
            q = request.POST.get("delname", "")
            que = Directors.objects.get(id=q)
            que.delete()
            return HttpResponseRedirect("/user/director")
    ...
    def del_manager(request):
        if request.method == "POST":
            q = request.POST.get("delname", "")
            que = Fund_manager.objects.get(id=q)
            que.delete()
            return HttpResponseRedirect("/user/manager")
    ...
    def update_director(request):
        if request.method == "POST":
            q = request.POST.get("upname", "")
            que = Directors.objects.get(id=q)
            que.users_id = Users.objects.get(id=request.POST.get("users_id"))
            que.phone = request.POST.get("phone")
